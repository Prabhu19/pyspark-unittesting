...
# returns the Spark context for each of the test...
...
# returns the Spark session for each of the test..
@pytest.fixture(scope="session")
def spark_session(spark_context):
    ...
    """
    return SparkSession.builder.master("local[2]").appName("Word Count").getOrCreate()
